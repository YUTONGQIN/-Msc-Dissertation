...
%perform (Wilcoxon rank sum test) based on the whole data (the whole data
%can be used here since we do not aim to train any classifier but just
%check feature importances)  
pvalue = [];
for i = 1:47201
pv = ranksum(sett(label==1,i),sett(label==-1,i),'method','approximate');
pvalue = [pvalue pv];
end
