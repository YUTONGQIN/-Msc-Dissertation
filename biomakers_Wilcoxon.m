...
%carry out Wilcoxon rank sum test in pre-processing to remove the most uninformative genes
[index1, Z] = rankfeatures(train_valid_set', train_valid_label,'Criterion', 'wilcoxon');

%select the first 100 features with larger standardized u-statistic
IDX_w = index1(1:100); 

for i = 1:5

%four folds are used as training data
train_set = train_valid_set(Indices~=i,IDX_w);
train_label = train_valid_label(Indices~=i);

%one fold left out is regarded as validation dataset
valid_set = train_valid_set(Indices==i,IDX_w);
...
%find the rank of the selected features in Wilcoxon rank sum test
rank_w = [];
for i = 1:length(final_features)
r = find(IDX_w==final_features(i));
rank_w = [rank_w r];
end

%calculate LOO errors using top-ranked 50 features selected by Wilcoxon rank 
%sum test 
final_set2 = train_valid_set(:,IDX_w(1:50));
...
%number of hits and 50 top-ranked features in Wilcoxon rank sum test
LOOE{ii} = LOOerr;
LOOE2{ii} = LOOerr2;
...
%record the rank of the selected features in Wilcoxon rank sum test  
Rank_in_ttest{ii} = rank_w;
...
hold on
plot(10,19,'.','MarkerSize',20)

%draw a plot of LOO errors (using 50 top-ranked features in Wilcoxon rank 
%sum test) obtained in the first fold in the outer loop
LOOerr2 = LOOE2{1};
...
genelabels2 = genes(final_features+4000);
...
%features
heatmap_data = transpose(test_set(:,final_features+4000));
cgo = clustergram(heatmap_data,'Standardize','none','RowPDist', {'correlation'},'ColumnPDist',{'correlation'})
...
%draw correlation heat map using 100 top-ranked features in Wilcoxon rank
%sum test 
figure;
%colormap('hot')
imagesc(corrcoef(tb_aftctl(:,IDX_w)'))
colorbar
